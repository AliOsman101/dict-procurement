...
protected function afterCreate(): void
{
    ...
    foreach ($modules as $module => $prefix) {
        ...
        if ($module === 'request_for_quotation') {
            $approver = $defaultApprovers->firstWhere('office_section', $this->record->office_section);
            if ($approver) {
                try {
                    ...
                    \Log::error("RFQ approval creation failed for procurement_id={$child->id}, employee_id={$approver->employee_id}: {$e->getMessage()}");
                }
            } else {
                \Log::warning("No RFQ approver found for office_section={$this->record->office_section}, procurement_id={$child->id}");
            }
        } else {
