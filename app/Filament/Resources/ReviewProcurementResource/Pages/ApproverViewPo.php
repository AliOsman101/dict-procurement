...
class ApproverViewPo extends ViewRecord
{
    ...
    {
        return "Purchase Order No. " . ($this->record->procurement_id ?? 'N/A');
    }
    ...
    protected function getHeaderActions(): array
    {
        ...
        if (!$canAct) {
            return [
                Action::make('viewPdf')
                    ...
                    ->url(fn () => route('procurements.po.pdf', $this->record), true)
                    ->color('info'),
            ];
        }
            ...
            Action::make('approve')
                ...
                ->action(function () use ($employeeId) {
                    ...
                    if ($approval) {
                        ...
                        $allApproved = $this->record->approvals()->where('module', 'purchase_order')
                            ->where('status', 'Pending')
                            ->doesntExist();
        ...
        \App\Helpers\ActivityLogger::log(
            ...
            'Purchase Order ' . $this->record->procurement_id . 
            ' was approved by ' . \Illuminate\Support\Facades\Auth::user()->name
        );
        ...
        \App\Helpers\ActivityLogger::log(
            ...
            'Purchase Order ' . $this->record->procurement_id . 
            ' was rejected by ' . \Illuminate\Support\Facades\Auth::user()->name . 
            ' with remarks: "' . $data['remarks'] . '"'
        );

                        Notification::make()->title('Purchase Order rejected')->danger()->send();
                        $this->record->refresh();
                    }
                }),
        ];
