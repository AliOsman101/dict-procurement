...
<head>
    ...
    <style>
        ...
        .signatory-name {
            ...
            overflow: visible;
            text-overflow: clip;
            min-width: 0; /* Allow name to expand */
...
<body>
    ...
    <!-- Signatories -->
    @php
        // Fetch all approvers for bac_resolution_recommending_award
        $approvers = \App\Models\DefaultApprover::where('module', 'bac_resolution_recommending_award')
            ->with('employee')
            ->get();

        // Get unique Regular Members (max 2), avoiding duplicates by employee_id
        $regularMembers = $approvers->whereIn('designation', ['Regular Member', 'Regular Member (Principal)'])
            ->unique('employee_id')
            ->values()
            ->take(2);

        // Assign other roles based on designation
        $provisionalMember = $approvers->whereIn('designation', ['Provisional Member', 'Provisional Member / End-User'])->first();
        $viceChairperson = $approvers->where('designation', 'Vice - Chairperson')->first(); // No fallback if missing
        $chairperson = $approvers->where('designation', 'Chairperson')->first();
    @endphp
        ...
        <tr>
            <td colspan="2" style="width:33.33%; text-align:center; border:none; padding: 10px 5px 5px 5px;">
                <div class="signatory-name" style="border-bottom:1px solid black; width:80%; margin:0 auto;">
                    <span style="font-weight:bold; text-transform:uppercase;">{{ $regularMembers->get(0)?->employee?->full_name ?? '' }}</span>
                </div>
                <i>Regular Member (Principal)</i>
            </td>
            <td colspan="2" style="width:33.33%; text-align:center; border:none; padding: 10px 5px 5px 5px;">
                <div class="signatory-name" style="border-bottom:1px solid black; width:80%; margin:0 auto;">
                    <span style="font-weight:bold; text-transform:uppercase;">{{ $regularMembers->get(1)?->employee?->full_name ?? '' }}</span>
                </div>
                <i>Regular Member (Principal)</i>
            </td>
            <td colspan="2" style="width:33.33%; text-align:center; border:none; padding: 10px 5px 5px 5px;">
