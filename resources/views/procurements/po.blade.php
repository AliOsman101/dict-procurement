...
<body>
    ...
    @php
        ...
        // Procurement Type: NP 53.9 for small value
    ...
    <!-- Signatories -->
    @php
        $regionalDirector = \App\Models\DefaultApprover::where('module', 'purchase_order')
            ->where('designation', 'Regional Director')
            ->with('employee')
            ->first();
        $regionalDirectorName = $regionalDirector?->employee 
            ? strtoupper($regionalDirector->employee->firstname . ' ' . 
                ($regionalDirector->employee->middlename ? substr($regionalDirector->employee->middlename, 0, 1) . '. ' : '') . 
                $regionalDirector->employee->lastname) 
            : 'ENGR. REYNALDO T. SY';
    @endphp
    <table style="width:100%; border-collapse: collapse; font-size:11px;">
        <tr>
            ...
            <td style="border-right:1px solid black; border-left:none; border-bottom:none; border-top:none; padding:30px 10px 5px 10px; text-align:center; width:50%; vertical-align:bottom;">
                <span style="text-decoration:underline;"><b>{{ $regionalDirectorName }}</b></span><br>
