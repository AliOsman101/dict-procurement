...
<head>
    ...
    <style>
        ...
        .s23 { white-space: normal; overflow: hidden; word-wrap: break-word; }
        .s24 { font-weight: bold; }
...
<body>
    <!-- Header with Logo -->
    <div style="text-align:center; margin:0; padding:0; line-height:1.2;">
        <img src="{{ public_path('images/dict-logo.png') }}" 
            alt="DICT Header" 
            style="width:50%; max-height:150px; margin:0 auto 4px auto; display:block;">
        
        <p style="margin:0; font-size:8pt; margin-bottom: 15px;">
    ...
    <table class="no-border">
        <tr style="height: 25px;"><td class="s3" colspan="26">REQUEST FOR QUOTATION RESPONSE</td></tr>
    ...
    <table style="font-size: 8pt; margin-left: 10px; margin-top: 15px;">
        ...
        <tr style="height: 19px;"><td class="s22" style="border-left: 2px solid #000000;"></td><td class="s21">2.</td><td class="s11" colspan="24">Professional License/CV (for Consulting Services)</td></tr>
        ...
        <tr style="height: 19px;"><td class="s22" style="border-left: 2px solid #000000;"></td><td class="s21">4.</td><td class="s11" colspan="24">PCAB License (for Infrastructure Projects)</td></tr>
        <tr style="height: 19px;"><td class="{{ $abc > 500000 ? 's20' : 's22' }}" style="border-left: 2px solid #000000;"></td><td class="s21">5.</td><td class="s11" colspan="24">Latest Income/Business Tax Return (for ABCs above Php 500,000.00);</td></tr>
        <tr style="height: 19px;"><td class="s20" style="border-left: 2px solid #000000;"></td><td class="s21">6.</td><td class="s11" colspan="24">Terms and Conditions for Contract of Service (catering) / Technical Specifications and Requirements (printing service).</td></tr>
    ...
    <!-- BAC Chairperson Line -->
    @php
        $bacChairperson = \App\Models\DefaultApprover::where('module', 'request_for_quotation')
            ->where('office_section', $procurement->office_section)
            ->with('employee')
            ->first();
        $bacChairpersonName = $bacChairperson && $bacChairperson->employee ? $bacChairperson->employee->full_name : 'Not set';
    @endphp
    <table style="width: 100%; margin-top: 15px; margin-bottom: 10px; border-collapse: collapse;">
        <tr>
            <td style="text-align: left; padding-top: 30px;">
                <div style="font-weight: bold; display: inline-block; width: 310px; text-align: center;">{{ $bacChairpersonName }}</div>
    ...
    <table style="width: 100%; border: 1px solid black; border-collapse: collapse; margin-top: 10px; margin-bottom: 20px; font-size: 8pt; text-align: center;">
        ...
        <tbody>
            @foreach($items as $index => $item)
                @php
                    $quote = $quotes->firstWhere('procurement_item_id', $item->id);
                @endphp
                <tr>
                    ...
                    <td style="border: 1px solid black; text-align: center;">
                        {{ $quote && $quote->statement_of_compliance ? '✓' : '' }}
                    </td>
                    <td style="border: 1px solid black; text-align: center;">
                        {{ $quote && !$quote->statement_of_compliance ? '✓' : '' }}
                    </td>
                    <td style="border: 1px solid black;">{{ $quote ? $quote->specifications : 'NA' }}</td>
                    <td style="border: 1px solid black;">₱{{ $quote ? number_format($quote->unit_value, 2) : 'NA' }}</td>
                    <td style="border: 1px solid black;">₱{{ $quote ? number_format($quote->total_value, 2) : 'NA' }}</td>
                </tr>
            @endforeach
            <tr>
    ...
    <table style="margin-top: 20px; width: 100%; border-collapse: collapse; font-size: 8pt;">
        ...
        <tr style="height: 25px;">
            ...
            <td colspan="7" style="vertical-align: bottom;">
                <div style="text-align: center;">{{ $rfqResponse->submitted_by ?? '' }}</div>
                <div style="border-bottom: 1px solid black; height: 15px;"></div>
            </td>
            <td colspan="2"></td>
            <td colspan="7" style="vertical-align: bottom; font-weight: bold;">
                Date :
                <span style="display:inline-block; border-bottom: 1px solid black; width: 120px; height: 15px; margin-left: 5px;">
                    {{ $rfqResponse->submitted_date ? \Carbon\Carbon::parse($rfqResponse->submitted_date)->format('F j, Y') : '' }}
                </span>
            </td>
            <td colspan="5"></td>
        </tr>
        ...
        <tr style="height: 25px;">
            ...
            <td colspan="7">
                <div style="border-bottom: 1px solid black;">{{ $rfqResponse->designation ?? '' }}</div>
            </td>
            <td colspan="2"></td>
            <td colspan="6" style="font-weight: bold;">TIN: ({{ $rfqResponse->vat ? '✓' : ' ' }}) VAT ({{ $rfqResponse->nvat ? '✓' : ' ' }}) NVAT :</td>
            <td colspan="6">
                <div style="border-bottom: 1px solid black;">{{ $rfqResponse->tin ?? '' }}</div>
            </td>
        </tr>
        ...
        <tr style="height: 25px;">
            ...
            <td colspan="7">
                <div style="border-bottom: 1px solid black;">{{ $rfqResponse->business_name ?? '' }}</div>
            </td>
            <td colspan="2"></td>
            <td colspan="6" style="font-weight: bold;">PhilGEPS Reg No. :</td>
            <td colspan="6">
                <div style="border-bottom: 1px solid black;">{{ $rfqResponse->philgeps_reg_no ?? '' }}</div>
            </td>
        </tr>
        ...
        <tr style="height: 25px;">
            ...
            <td colspan="7">
                <div style="border-bottom: 1px solid black;">{{ $rfqResponse->business_address ?? '' }}</div>
            </td>
            <td colspan="2"></td>
            <td colspan="6" style="font-weight: bold;">LBP Account Name :</td>
            <td colspan="6">
                <div style="border-bottom: 1px solid black;">{{ $rfqResponse->lbp_account_name ?? '' }}</div>
            </td>
        </tr>
        ...
        <tr style="height: 25px;">
            ...
            <td colspan="7">
                <div style="border-bottom: 1px solid black;">{{ $rfqResponse->contact_no ?? '' }}</div>
            </td>
            <td colspan="2"></td>
            <td colspan="6" style="font-weight: bold;">LBP Account Number :</td>
            <td colspan="6">
                <div style="border-bottom: 1px solid black;">{{ $rfqResponse->lbp_account_number ?? '' }}</div>
            </td>
        </tr>
        <!-- Email Address -->
        <tr style="height: 25px;">
            ...
            <td colspan="7">
                <div style="border-bottom: 1px solid black;">{{ $rfqResponse->email_address ?? '' }}</div>
            </td>
            <td colspan="14"></td>
        </tr>
    </table>
