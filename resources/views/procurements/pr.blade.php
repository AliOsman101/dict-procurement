...
@php
    $iiddFocal = \App\Models\DefaultApprover::where('module', 'purchase_request')
        ->where('designation', 'IIDB Focal Person')
        ->with('employee')
        ->first();
    $regionalDirector = \App\Models\DefaultApprover::where('module', 'purchase_request')
        ->where('designation', 'Regional Director')
        ->with('employee')
        ->first();
    $budgetOfficer = \App\Models\DefaultApprover::where('module', 'purchase_request')
...
<table style="width:100%; border:1px solid rgb(246, 246, 246); border-collapse:collapse; margin-top:0px; font-size:8px;">
    <tr>
        ...
        <td style="width:50%; padding:6px; vertical-align:top; border-right:none;">
            <span class="italic-label">Requested By:</span><br><br>
            <!-- Signature -->
            <div style="margin-top:4px;">
                <span style="display:inline-block; width:90px;">Signature:</span>
                <span style="display:inline-block; width:220px; border-bottom:1px solid black; height:16px;"></span>
            </div>
            <!-- Printed Name -->
            <div style="margin-top:8px;">
                <span style="display:inline-block; width:90px;">Printed Name:</span>
                <span style="display:inline-block; width:220px; border-bottom:1px solid black; text-align:center; font-weight:bold;">
                    {{ $iiddFocal?->employee?->firstname . ' ' . ($iiddFocal?->employee?->middlename ?? '') . ' ' . $iiddFocal?->employee?->lastname }}
                </span>
            </div>
            <!-- Designation -->
            <div style="margin-top:8px;">
                <span style="display:inline-block; width:90px;">Designation:</span>
                <span style="display:inline-block; width:220px; border-bottom:1px solid black; text-align:center;">
                    IIDB Focal Person
                </span>
            </div>
        </td>
        ...
        <td style="width:50%; padding:6px; vertical-align:top; border-left:none;">
            <span class="italic-label">Approved By:</span><br><br>
            <!-- Signature -->
            <div style="margin-top:4px;">
                <span style="display:inline-block; width:90px; visibility:hidden;">Signature:</span>
                <span style="display:inline-block; width:220px; border-bottom:1px solid black; height:16px;"></span>
            </div>
            <!-- Printed Name -->
            <div style="margin-top:8px;">
                <span style="display:inline-block; width:90px; visibility:hidden;">Printed Name:</span>
                <span style="display:inline-block; width:220px; border-bottom:1px solid black; text-align:center; font-weight:bold;">
                    {{ $regionalDirector?->employee?->firstname . ' ' . ($regionalDirector?->employee?->middlename ?? '') . ' ' . $regionalDirector?->employee?->lastname }}
                </span>
            </div>
            <!-- Designation -->
            <div style="margin-top:8px;">
                <span style="display:inline-block; width:90px; visibility:hidden;">Designation:</span>
                <span style="display:inline-block; width:220px; border-bottom:1px solid black; text-align:center;">
                    Regional Director
                </span>
            </div>
...
<div style="margin-top:30px;" class="center">
    <div class="purple">FUNDS AVAILABLE</div>
    <br>
    <strong>
        {{ $budgetOfficer?->employee?->firstname . ' ' . ($budgetOfficer?->employee?->middlename ?? '') . ' ' . $budgetOfficer?->employee?->lastname }}
    </strong><br>
    <i>Budget Officer</i>
</div>
